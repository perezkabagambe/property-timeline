...
replaced the old `.props-grid` card-only layout. IMD, nearest train stations,
and nearest hospitals render three-across in a second `.near-row` right after
`.schools-row` (in that left-to-right order) — the generic `.near-row .near-block`
180px flex-basis (not `.schools-row`'s wider 300px) suits these three compact
blocks. On narrow viewports `.near-row` wraps to a single column, so the stacked
mobile order is exactly: property card, primary, secondary, IMD, stations,
hospitals.
